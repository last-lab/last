...
from last.services.models import AbstractAdmin, AbstractPermission, AbstractResource
...
class PermissionProvider(Provider):
    ...
    async def check(cls, admin: AbstractAdmin, resource: str, permission: enums.Permission) -> bool:
        ...
        for role in admin.roles:
            ...
            if await role.permissions.filter(
                resource__iexact=resource, permission=permission
            ).exists():
                has_permission = True
                break
