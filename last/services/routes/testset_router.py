...
from last.types.db_models import TestSetDB
from last.types.testset_model import (
    DeleteTestsets,
    NewTestset,
...
async def import_testset(
    ...
    try:
        ...
        if response.status_code != 200:
            raise HTTPException(status_code=400, detail="Failed to fetch testset from endpoint")
...
async def get_testsets(
    ...
    # Define query expression
    query_expression = query.eq(TestSetDB.user, user.id) & query.eq(TestSetDB.app_name, app_name)
...
async def get_testset(
    ...
    if testset is not None:
        ...
        raise HTTPException(status_code=404, detail=f"testset with id {testset_id} not found")
...
async def delete_testsets(
    ...
    for testset_id in delete_testsets.testset_ids:
        ...
        if testset is not None:
            ...
            raise HTTPException(status_code=404, detail=f"testset {testset_id} not found")
