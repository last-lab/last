...
from last.services.system.db_manager import get_templates, get_user_object
from last.services.system.docker_utils import restart_container
from last.services.system.selectors import get_user_and_org_id
from last.types.api_models import URI, Image, RestartAppContainer, Template
...
async def pull_image(
    ...
    try:
        image_res = await pull_image_from_docker_hub(f"{repo_owner}/{repo_name}", image_name)
    except DockerError as ext:
        return JSONResponse({"message": "Image with tag does not exist", "meta": str(ext)}, 404)

    # Get data from image response
    image_tag_name = image_res[0]["id"]
    image_id = await get_image_details_from_docker_hub(repo_owner, repo_name, image_tag_name)
