...
from last.services.system.auth_helper import SessionContainer, verify_session
from last.services.system.db_manager import engine, get_user_object, query
from last.services.system.evaluation_service import (
...
from last.services.system.results_service import (
...
async def fetch_evaluation_scenarios(
    ...
    query_expression = query.eq(EvaluationScenarioDB.evaluation_id, evaluation_id) & query.eq(
        EvaluationScenarioDB.user, user.id
    )
...
async def create_evaluation_scenario(
    ...
    # Get user and organization id
    kwargs: dict = await get_user_and_org_id(stoken_session)
    result = await create_new_evaluation_scenario(evaluation_id, evaluation_scenario, **kwargs)
...
@router.put("/{evaluation_id}/evaluation_scenario/{evaluation_scenario_id}/{evaluation_type}")
...
async def delete_evaluations(
    ...
    for evaluations_id in delete_evaluations.evaluations_ids:
        # Construct query expression builder
        query_expression = query.eq(EvaluationDB.id, ObjectId(evaluations_id)) & query.eq(
            EvaluationDB.user, user.id
        )
        evaluation = await engine.find_one(EvaluationDB, query_expression)
