...
from last.types.db_models import (
    EvaluationDB,
...
async def create_new_evaluation(payload: NewEvaluation, **kwargs: dict) -> Dict:
    ...
    evaluation_type_settings = EvaluationTypeSettings(
        similarity_threshold=0.0 if similarity_threshold is None else similarity_threshold
    )
    ...
    for datum in csvdata:
        try:
            inputs = [{"input_name": name, "input_value": datum[name]} for name in payload.inputs]
        except KeyError:
    ...
    **kwargs,
    ...
    if (
        ...
        evaluation = evaluate_with_ai_critique(
            ...
            llm_app_inputs=[
                scenario_input.dict() for scenario_input in current_evaluation_scenario.inputs
            ],
