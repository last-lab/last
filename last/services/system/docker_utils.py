...
from last.types.api_models import URI, AppVariant, DockerEnvVars, Image
...
def list_images() -> List[Image]:
    ...
    These are tagged with the registry name (config in both last and agenta-cli)
...
def start_container(
    ...
    try:
        ...
        labels = {
            ...
            f"traefik.http.routes.{user_backend_container_name}.middlewares": f"{user_backend_container_name}-strip-prefix",
            f"traefik.http.routes.{user_backend_container_name}.service": f"{user_backend_container_name}",
        }
        ...
        if os.environ["ENVIRONMENT"] == "production":
            ...
            production_labels = {
                f"traefik.http.routes.{user_backend_container_name}.rule": f"Host(`{os.environ['BARE_DOMAIN_NAME']}`) && PathPrefix(`/{user_backend_url_path}`)",
            }
            ...
            if "https" in os.environ["DOMAIN_NAME"]:
                # SSL specific labels
                ssl_labels = {
                    f"traefik.http.routes.{user_backend_container_name}.entrypoints": "web-secure",
                    f"traefik.http.routes.{user_backend_container_name}.tls": "true",
                    f"traefik.http.routes.{user_backend_container_name}.tls.certresolver": "myResolver",
                }
                labels.update(ssl_labels)
        else:
            # Development specific labels
            development_labels = {
                f"traefik.http.routes.{user_backend_container_name}.rule": f"PathPrefix(`/{user_backend_url_path}`)",
                f"traefik.http.routes.{user_backend_container_name}.entrypoints": "web",
            }
        ...
        return URI(uri=f"http://{os.environ['BARE_DOMAIN_NAME']}/{user_backend_url_path}")
...
def restart_container(container_id: str):
    ...
    try:
        ...
        print("Err type ---> ", ex)
        logger.error(f"Error restarting container with id: {container.id}. Error: {str(ex)}")
