...
from last.services.system import helpers
from last.types.api_models import App, AppVariant, Image, ImageExtended, Template
from last.types.converters import (
    app_variant_db_to_pydantic,
    image_db_to_pydantic,
    templates_db_to_pydantic,
)
from last.types.db_engine import DBEngine
from last.types.db_models import AppVariantDB, ImageDB, OrganizationDB, TemplateDB, UserDB
...
async def add_variant_based_on_image(app_variant: AppVariant, image: Image, **kwargs: dict):
    ...
    already_exists = any(
        [
            ...
            if av.app_name == app_variant.app_name and av.variant_name == app_variant.variant_name
        ]
    )
    if already_exists:
...
async def add_variant_based_on_previous(
    ...
    **kwargs: dict,
    ...
    query_expression = query.eq(AppVariantDB.app_name, previous_app_variant.app_name) & query.eq(
        AppVariantDB.variant_name, previous_app_variant.variant_name
    )

    # get the template variant to base the new one on
    template_variant: AppVariantDB = await engine.find_one(AppVariantDB, query_expression)
    ...
    if template_variant is None:
        ...
        raise ValueError("Template app variant is not a template, it is a forked variant itself")

    soft_deleted_app_variants = await list_app_variants(show_soft_deleted=True, **kwargs)
    already_exists = any(
        [
            av
            for av in soft_deleted_app_variants
            if av.app_name == previous_app_variant.app_name and av.variant_name == new_variant_name
        ]
    )
    if already_exists:
        raise ValueError("App variant with the same name already exists")
...
async def list_app_variants(
    ...
    app_variants: List[AppVariant] = [app_variant_db_to_pydantic(av) for av in app_variants_db]
    return app_variants
...
async def update_variant_parameters(
    ...
    elif db_app_variant.parameters is not None and set(db_app_variant.parameters.keys()) != set(
        parameters.keys()
    ):
        logger.error(
...
async def get_user_organization(user_id: str) -> OrganizationDB:
    ...
    organization = await engine.find_one(OrganizationDB, OrganizationDB.id == user.organization_id)
    return organization
...
async def get_user_image_instance(user_id: str, docker_id: str) -> ImageDB:
    ...
    query_expression = query.eq(ImageDB.user_id, user_id) & query.eq(ImageDB.docker_id, docker_id)
