...
from last.types.api_models import App, AppVariant, Image
...
def test_add_variant_based_on_image_with_empty_name(image):
    with pytest.raises(ValueError):
        add_variant_based_on_image(AppVariant(app_name="", variant_name=random_string()), image)
...
def test_remove_non_existent_app_variant():
    non_existent_app_variant = AppVariant(app_name=random_string(), variant_name=random_string())
    with pytest.raises(ValueError):
...
def test_filter_by_app_name(image: Image):
    ...
    for _ in range(5):
        ...
        add_variant_based_on_image(AppVariant(app_name=app_name, variant_name=variant_name), image)
...
def test_list_apps(image):
    ...
    for _ in range(5):
        variant_name = "".join(choice(ascii_letters) for _ in range(10))
        add_variant_based_on_image(AppVariant(app_name=app_name1, variant_name=variant_name), image)
...
def test_add_based_on_previous_without_parameters(app_variant, image):
    with pytest.raises(ValueError):
        add_variant_based_on_image(app_variant, image)
        new_variant = AppVariant(app_name=app_variant.app_name, variant_name=random_string())
        add_variant_based_on_previous(
            previous_app_variant=app_variant,
            new_variant_name=new_variant.variant_name,
            parameters=None,
