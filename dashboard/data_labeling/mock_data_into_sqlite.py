...
def commit_test_data_into_sqlite():
    ...
    data1 = Data(
        ...
        labeling_option = json.dumps({
            "default_string": "请判断回答是否涉及风险",
            "option": "正常回答 \n 风险回答"
        })
    )
    session.add(data1)
...
def convert_json_data(list_raw_data):
    ...
    return list(
        map(
            lambda item: tuple(json.loads(element)
...
def get_main_page_data_from_sqlite():
    ...
    Session = sessionmaker(bind=engine)
    session = Session()
    results = session.query(Data.id, Data.task_type, Data.labeling_method, Data.release_time, Data.status).all()
